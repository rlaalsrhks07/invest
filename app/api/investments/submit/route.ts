...
import { ASSETS, getRoundById, getTeamById } from "@/lib/gameData";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rawInvestments = body.investments;
    ...
    if (!Array.isArray(rawInvestments)) {
      return NextResponse.json(
        { error: "투자 내역이 올바르지 않습니다." },
        { status: 400 }
      );
    }

    const validAssetIds = new Set(ASSETS.map((asset) => asset.id));
    const mergedInvestments = new Map<string, number>();

    for (const rawItem of rawInvestments as InvestmentInput[]) {
      ...
      mergedInvestments.set(
        assetId,
        (mergedInvestments.get(assetId) ?? 0) + amount
      );
    }

    const investments = Array.from(mergedInvestments.entries()).map(
      ([assetId, amount]) => ({ assetId, amount })
    );

    if (investments.length === 0) {
      return NextResponse.json(
        { error: "최소 하나 이상의 산업에 투자해야 합니다." },
        { status: 400 }
      );
    }
    ...
    if (existingError) {
      console.error("기존 투자 확인 실패:", existingError);
      return NextResponse.json(
        { error: "기존 투자 내역 확인에 실패했습니다." },
        { status: 500 }
      );
    }
    ...
    if (!Number.isSafeInteger(currentCash) || currentCash < 0) {
      return NextResponse.json(
    ...
    if (totalAmount > currentCash) {
      return NextResponse.json(
        { error: "총 투자 금액이 현재 보유 자금을 초과합니다." },
        { status: 400 }
      );
    }
    ...
    if (insertError) {
      console.error("투자 내역 저장 실패:", insertError);
      return NextResponse.json(
        { error: "투자 내역 저장에 실패했습니다." },
        { status: 500 }
      );
    }

    const remainingCash = currentCash - totalAmount;
    ...
    if (updateCashError || !updatedTeam) {
      console.error("투자금 차감 실패:", updateCashError);
      ...
      if (rollbackError) {
        console.error("투자 내역 복구 실패:", rollbackError);
      }
    ...
    console.error("투자 제출 API 오류:", error);
    return NextResponse.json(
      { error: "서버 오류가 발생했습니다." },
      { status: 500 }
    );
